        ...
        {transactions.length === 0 ? (
          <Empty className="flex h-full items-center justify-center p-0">
            <EmptyHeader>
              ...
              <EmptyTitle>Переводов нет</EmptyTitle>
              <EmptyDescription>
                Вы еще не создали ни одного перевода.
              </EmptyDescription>
            </EmptyHeader>
          </Empty>
        ) : (
          <ScrollArea className="h-full">
            <ul className="flex flex-col gap-2">
              {transactions.map((transaction) => {
                ...
                const transactionMenuItems: MenuItem[] = [
                  {
                    dropdownItem: (
                ...
                return (
                  <DropdownMenu key={transaction.id}>
                    ...
                    <DropdownMenuContent className="w-auto">
                      <MenuItems type="dropdown" items={transactionMenuItems} />
                    </DropdownMenuContent>
                  </DropdownMenu>
                )
              })}
...
export function TransactionsCardSkeleton() {
  return (
    <Card className="h-full">
      <CardHeader>
...
type TransactionsCardErrorProps = {
  status?: string | number
  message?: string
  isRetrying?: boolean
  onRetry?: () => void
}
export function TransactionsCardError({
  status,
  message,
}: TransactionsCardErrorProps) {
  return (
    <Card className="h-full">
      <CardHeader>
        {status && (
